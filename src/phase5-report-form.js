...
const galleryIcon = '<svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="16" rx="2"/><circle cx="8.5" cy="9" r="1.5"/><path d="m5 17 4.2-4 3 2.8 2.1-2.1L19 18"/></svg>';
const snowIcon = '<svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2v20M4.9 6l14.2 12M19.1 6 4.9 18M5 12h14"/></svg>';
const trackIcon = '<svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M4 17h16M6 13h12M8 9h8M10 5h4"/></svg>';
const skateIcon = '<svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M5 6h9l2 8h3"/><path d="M4 18h16M7 18v2M17 18v2"/><path d="M9 6v8"/></svg>';
...
function renderForm(ride, target = document.querySelector('.welcome-card'), existingReport = ride.report || null, modal = false) {
  ...
  card.innerHTML = `<div class="online-badge"><span></span> ONLINE</div><div class="eyebrow">${existingReport ? 'ÚPRAVA REPORTU' : 'JÍZDA DOKONČENA'}</div><h1>${existingReport ? 'Uprav report.' : 'Jaká byla stopa?'}</h1><p class="report-intro">${existingReport ? 'Report ještě nebyl odeslán. Můžeš ho upravit.' : `Ještě mi řekni, jak to dneska vypadalo. Zaznamenal jsem ${km} km.`}</p>${existingReport ? `<div class="report-edit-deadline">Report můžeš upravovat ještě ${formatRemaining(Number(existingReport.editableUntil) - Date.now())}.</div>` : ''}
    <form id="phase5ReportForm" class="report-form">
      ...
      <section class="report-section photo-report-section"><div class="report-section-title"><span class="report-icon photo-report-icon" aria-hidden="true">${cameraIcon}</span><div><h2>Fotky</h2><small>Přidej jednu nebo více fotek.</small></div></div><button type="button" class="photo-add-button" id="endPhotoAdd"><span class="button-icon">${cameraIcon}</span> Přidat fotku</button><div class="photo-count" id="photoCount"></div></section>
...
function openPhotoChoice(ride) {
  ...
  modal.innerHTML = `<div class="photo-choice-card" role="dialog" aria-modal="true" aria-label="Přidat fotku"><div class="photo-choice-header"><strong>Přidat fotku</strong><button type="button" class="photo-choice-close" aria-label="Zavřít">×</button></div><button type="button" class="photo-choice-option" id="photoChoiceCamera"><span class="button-icon">${cameraIcon}</span> Vyfotit</button><button type="button" class="photo-choice-option gallery" id="photoChoiceGallery"><span class="button-icon">${galleryIcon}</span> Z galerie <small>můžeš vybrat více fotek</small></button></div>`;
...
async function handleEndPhotoFiles(files, ride) {
  ...
  finally { const b=document.querySelector('#endPhotoAdd'); if(b){b.disabled=false;b.innerHTML=`<span class="button-icon">${cameraIcon}</span> Přidat fotku`;} const s=document.querySelector('.save-report'); if(s) s.disabled=false; }
}
