...
export class RideEngine {
  ...
  start(initialPosition = null) { if (this.isRunning) return; this.reset(); this.startedAt = Date.now(); this.isRunning = true; this.isPaused = false; const initial = this.normalizePosition(initialPosition); if (initial) { this.lastPosition = initial; this.distanceAnchor = initial; this.lastDistancePosition = initial; this.trackPoints.push(initial); this.rawTrackPoints.push(initial); this.diagnostics.acceptedPoints += 1; } this.startGpsWatch(); this.startClock(); this.emit(); }
  restoreFromRecord(record) { this.destroy(); this.reset(); const points = Array.isArray(record?.trackPoints) ? record.trackPoints : []; this.trackPoints = points.map(point => ({ ...point })); this.rawTrackPoints = Array.isArray(record?.rawTrackPoints) ? record.rawTrackPoints.map(point => ({ ...point })) : this.trackPoints.map(point => ({ ...point })); this.lastPosition = this.trackPoints.at(-1) || null; this.distanceAnchor = this.lastPosition; this.lastDistancePosition = this.lastPosition; this.totalDistanceM = Math.max(0, Number(record?.distanceM) || 0); this.rawDistanceM = Math.max(0, Number(record?.diagnostics?.rawDistanceM) || this.totalDistanceM); this.diagnostics = { ...this.diagnostics, ...(record?.diagnostics || {}), filteredDistanceM: this.totalDistanceM, rawDistanceM: this.rawDistanceM }; this.startedAt = Number(record?.startedAt) || Date.now(); this.pausedDurationMs = Math.max(0, Number(record?.pausedDurationMs) || 0) + Math.max(0, Date.now() - (Number(record?.endedAt) || Date.now())); this.isRunning = true; this.isPaused = false; this.startGpsWatch(); this.startClock(); this.emit(); }
  ...
  stop() { if (!this.isRunning) return this.getSnapshot(); const endedAt = Date.now(), pausedExtra = this.isPaused ? endedAt - this.pausedAt : 0; const totalPausedDurationMs = this.pausedDurationMs + pausedExtra; const activeTimeMs = Math.max(0, endedAt - this.startedAt - totalPausedDurationMs); this.stopGpsWatch(); this.stopClock(); this.isRunning = false; this.isPaused = false; this.diagnostics.rawDistanceM = this.rawDistanceM; this.diagnostics.filteredDistanceM = this.totalDistanceM; const result = { distanceM: this.totalDistanceM, rawDistanceM: this.rawDistanceM, activeTimeMs, pausedDurationMs: totalPausedDurationMs, startedAt: this.startedAt, endedAt, trackPoints: this.trackPoints.map(point => ({ ...point })), rawTrackPoints: this.rawTrackPoints.map(point => ({ ...point })), diagnostics: { ...this.diagnostics, stoppedAt: endedAt } }; this.emit(); return result; }
  destroy() { this.stopGpsWatch(); this.stopClock(); document.removeEventListener('visibilitychange', this.handleVisibilityChange); }
  reset() { this.stopGpsWatch(); this.stopClock(); this.lastPosition = null; this.trackPoints = []; this.rawTrackPoints = []; this.totalDistanceM = 0; this.rawDistanceM = 0; this.distanceAnchor = null; this.lastDistancePosition = null; this.startedAt = 0; this.pausedAt = 0; this.pausedDurationMs = 0; this.lastError = null; this.diagnostics = { visibilityEvents: [], gpsErrors: [], watchRestarts: 0, gpsCallbacks: 0, acceptedPoints: 0, rejectedPoints: 0, rejectedSegments: 0, maxGapMs: 0, maxObservedSpeedKmh: 0, distanceSegments: 0, rawDistanceM: 0, filteredDistanceM: 0 }; }
  ...
  handlePosition(position) {
    if (!this.isRunning || this.isPaused) return; const current = this.normalizePosition(position); if (!current) return; this.diagnostics.gpsCallbacks += 1; this.lastError = null; this.rawTrackPoints.push(current);
    const previous = this.lastPosition;
    if (!previous) { this.lastPosition = current; this.distanceAnchor = current; this.lastDistancePosition = current; this.trackPoints.push(current); this.diagnostics.acceptedPoints += 1; this.emit(); return; }
    const segment = haversineMeters(previous, current), elapsedMs = Math.max(1, current.timestamp - previous.timestamp), gapMs = Math.max(0, current.timestamp - previous.timestamp), speedKmh = segment / 1000 / (elapsedMs / 3600000);
    this.diagnostics.maxGapMs = Math.max(this.diagnostics.maxGapMs, gapMs); if (Number.isFinite(speedKmh)) this.diagnostics.maxObservedSpeedKmh = Math.max(this.diagnostics.maxObservedSpeedKmh, speedKmh);
    const accuracy = Math.max(previous.accuracy || 0, current.accuracy || 0); const polylineValid = accuracy <= MAX_POLYLINE_ACCURACY_M && gapMs <= MAX_CONNECT_GAP_MS && segment <= MAX_HARD_SEGMENT_M && speedKmh <= MAX_ROLLER_SPEED_KMH;
    if (polylineValid) { this.trackPoints.push(current); this.diagnostics.acceptedPoints += 1; } else { this.diagnostics.rejectedPoints += 1; this.diagnostics.rejectedSegments += 1; }
    if (gapMs > MAX_CONNECT_GAP_MS || speedKmh > MAX_ROLLER_SPEED_KMH || segment > MAX_HARD_SEGMENT_M || accuracy > MAX_DISTANCE_ACCURACY_M) { this.distanceAnchor = current; this.lastDistancePosition = current; this.lastPosition = current; this.emit(); return; }
    this.rawDistanceM += segment;
    const movementThreshold = Math.max(MIN_DISTANCE_MOVEMENT_M, Math.min(20, accuracy * 0.25)); const fromAnchor = this.distanceAnchor ? haversineMeters(this.distanceAnchor, current) : segment;
    if (fromAnchor >= movementThreshold) { this.totalDistanceM += fromAnchor; this.diagnostics.distanceSegments += 1; this.distanceAnchor = current; }
    this.lastDistancePosition = current; this.lastPosition = current; this.diagnostics.rawDistanceM = this.rawDistanceM; this.diagnostics.filteredDistanceM = this.totalDistanceM; this.emit();
  }
  ...
  normalizePosition(position) { if (!position?.coords) return null; const { latitude, longitude, accuracy } = position.coords; if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return null; return { latitude, longitude, accuracy: Number.isFinite(accuracy) ? accuracy : 999, timestamp: position.timestamp || Date.now() }; }
