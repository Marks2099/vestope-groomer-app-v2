...
function installStyles() {
  ...
  style.textContent = `
    ...
    .app-version-footer{display:inline-flex;align-items:center;justify-content:center;gap:7px;flex-wrap:wrap}
    .app-version-status{font-weight:800}.app-version-status.current{color:#28754a}.app-version-status.checking{color:#61758d}.app-version-status.offline{color:#7b8da1}.app-version-status.update{color:#1769aa}
    .app-version-mark{width:15px;height:15px;display:inline-grid;place-items:center;border-radius:50%;font-size:10px;font-weight:900;line-height:1;background:#20b55a;color:#fff}
    .app-version-status.offline .app-version-mark{background:#7b8da1}.app-version-status.update .app-version-mark{background:#1769aa}.app-version-status.checking .app-version-mark{background:#9aabbc}
...
function render(status = lastStatus) {
  ...
  bar.innerHTML=`<span class="app-version-footer"><span>VeStope.cz</span><span>·</span><span>verze ${APP_VERSION}</span><span>·</span><span class="app-version-status ${cls}"><span class="app-version-mark">${icon}</span> ${label}</span></span>`;
}
...
async function checkVersion(){
  ...
  }catch(_){lastStatus=navigator.onLine?'checking':'offline';}
  render();
}
