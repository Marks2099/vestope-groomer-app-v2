...
function installStyles(){
  ...
  style.textContent=`
    ...
    .groomed-line{position:absolute;left:0;width:100%;border-radius:99px}.groomed-line.thin{height:2px;background:#afc8da}.groomed-line.thick{height:5px;background:#8faec5}
...
function sync(){
  const card=document.querySelector('.ride-card');if(!card)return;installStyles();
...
installStyles();window.__vestopeSyncRideAnimation=sync;new MutationObserver(sync).observe(document.body,{childList:true,subtree:true});setTimeout(sync,0);
