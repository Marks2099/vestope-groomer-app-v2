...
export function installGroomerProfile() {
  installStyles();
  const observer = new MutationObserver(() => ensureAppControls());
...
function ensureAppControls() {
  ...
  if (!topbar) {
    const online = card.querySelector('.online-badge');
    if (!online) return;
    topbar = document.createElement('div');
    topbar.className = 'app-topbar';
    online.replaceWith(topbar);
    topbar.appendChild(online);
  }

  if (!topbar.querySelector('#groomerSummaryButton')) {
  ...
  const footer = document.querySelector('body > footer, #app + footer, footer');
  if (footer && !footer.querySelector('#logoutButton')) {
...
async function handleLogout() {
  ...
  if (button) { button.disabled = true; button.textContent = 'ODHLAŠUJI…'; }
  try {
    if (typeof window.__vestopeLogout !== 'function') throw new Error('Odhlášení není dostupné.');
    await window.__vestopeLogout();
  } catch (error) {
    if (button) { button.disabled = false; button.textContent = 'ODHLÁSIT SE'; }
    console.error(error);
    alert('Odhlášení se nepodařilo. Zkus stránku obnovit.');
  }
}
