...
const state = {
  ...
  ride: null, lastRide: null, locationMatch: null, rideId: null, photos: [], photoBusy: false,
};

const rideEngine = new RideEngine({
  onUpdate(snapshot) { if (state.phase === 'ride') { state.ride = snapshot; render(); } },
  onGpsError() { if (state.phase === 'ride') { state.ride = rideEngine.getSnapshot(); render(); } },
});
...
function renderRide() {
  ...
  return `<div class="online-badge"><span></span> ONLINE</div><div class="ride-header"><div class="eyebrow">JÍZDA PROBÍHÁ</div><h1>${paused ? 'Jízda je pozastavená.' : 'Jedu.'}</h1><p>${paused ? 'GPS i čas se nyní nezapočítávají.' : 'Živě sleduji tvoji trasu.'}</p></div><div class="ride-live-stats" aria-live="polite"><div class="live-stat"><span class="live-stat-icon" aria-hidden="true">⌁</span><strong>${distance}</strong><small>km</small></div><div class="live-stat"><span class="live-stat-icon" aria-hidden="true">◷</span><strong>${time}</strong><small>čas</small></div></div>${gpsMessage}<div class="ride-state ${paused ? 'paused' : 'running'}"><span class="ride-state-dot"></span>${paused ? 'PAUZA' : 'JÍZDA AKTIVNÍ'}</div><div class="ride-photo-row"><button class="ride-photo-button" id="ridePhotoButton" type="button" ${state.photoBusy ? 'disabled' : ''}>📷 ${photoText}</button><input id="ridePhotoInput" class="photo-input" type="file" accept="image/*" capture="environment" /></div><div class="ride-actions"><button class="ride-action pause-action" id="pauseButton" type="button">${paused ? 'POKRAČOVAT' : 'PAUZA'}</button><button class="ride-action stop-action" id="stopButton" type="button">UKONČIT JÍZDU</button></div><div class="ride-location">${formatPosition(ride.position || state.position)}</div>`;
}
...
function startRide() {
  ...
  state.rideId = crypto.randomUUID(); state.photos = []; state.photoBusy = false;
  rideEngine.start(state.position); state.ride = rideEngine.getSnapshot(); state.phase = 'ride'; render();
}

function togglePause() { if (rideEngine.getSnapshot().isPaused) rideEngine.resume(); else rideEngine.pause(); state.ride = rideEngine.getSnapshot(); render(); }
...
async function handlePhotoInput(event) {
  ...
  if (!file || state.photoBusy || !state.rideId) return;
  state.photoBusy = true; render();
  try {
    const ride = rideEngine.getSnapshot();
    const position = await getCurrentPhotoPosition(ride.position || state.position);
    const trackPoints = rideEngine.getTrackPoints();
    const nearestTrackPoint = findNearestTrackPoint(position, trackPoints);
    const location = position ? await detectStartLocation(position, DEFAULT_AREA_ID) : null;
    const metadata = await addRidePhoto({
      rideId: state.rideId, file, capturedAt: Date.now(), position,
      nearestTrackPoint,
      nearestKnownStart: location?.nearestStart ? { id: location.nearestStart.id, name: location.nearestStart.name, distanceM: location.distanceToNearestStartM } : null,
    });
    state.photos.push(metadata);
  } catch (error) {
    state.error = error?.message || 'Fotku se nepodařilo uložit.';
  } finally { state.photoBusy = false; render(); }
}

function getCurrentPhotoPosition(fallback) {
  if (!('geolocation' in navigator)) return Promise.resolve(fallback || null);
  return new Promise((resolve) => {
    let done = false;
    const finish = (value) => { if (done) return; done = true; resolve(value || fallback || null); };
    const timeout = window.setTimeout(() => finish(fallback), 5000);
    navigator.geolocation.getCurrentPosition((position) => { clearTimeout(timeout); finish(position); }, () => { clearTimeout(timeout); finish(fallback); }, { enableHighAccuracy: true, timeout: 4500, maximumAge: 5000 });
  });
}

function findNearestTrackPoint(position, points) {
  if (!position || !Array.isArray(points) || !points.length) return null;
  const target = { latitude: Number(position.coords?.latitude ?? position.latitude), longitude: Number(position.coords?.longitude ?? position.longitude) };
  let nearest = null, best = Infinity;
  for (const point of points) {
    const distance = haversineMeters(target, point);
    if (distance < best) { best = distance; nearest = { ...point, distanceM: Math.round(distance) }; }
  }
  return nearest;
}
function haversineMeters(a, b) { const r = 6371000, p = Math.PI / 180, lat1 = a.latitude * p, lat2 = b.latitude * p, dLat = (b.latitude - a.latitude) * p, dLon = (b.longitude - a.longitude) * p; const h = Math.sin(dLat / 2) ** 2 + Math.cos(lat1) * Math.cos(lat2) * Math.sin(dLon / 2) ** 2; return 2 * r * Math.asin(Math.sqrt(h)); }
...
async function stopRide() {
  ...
  const photos = await listRidePhotos(state.rideId);
  const record = createRideRecord(result, {
    id: state.rideId,
    photos: photoMetadataForRide(photos),
  ...
  state.lastRide = await saveRide(record); state.phase = 'rideSummary'; render();
}
