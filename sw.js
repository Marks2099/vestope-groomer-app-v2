const CACHE_NAME = 'vestope-groomer-v2-shell-v23';
...
self.addEventListener('install',event=>{event.waitUntil(caches.open(CACHE_NAME).then(async cache=>{await cache.addAll(SHELL);await cacheRemoteAssets(cache);}));});
